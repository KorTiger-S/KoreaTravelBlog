...
- 대기 중인 초안이 없고 사용자의 주제 요청도 없으면(승인 완료 후), **이번 달 마지막 3일인지부터 확인**합니다:
  (아래 월말 축제 → 격주 행사 → 화요일 여행지 테마 → 수요일 맛집/음식 순서로 확인하고, 전부
  해당 없을 때만 여행 팁으로 넘어갑니다.)
  ```
...
- 격주 행사 로스터도 해당 없으면, **오늘이 화요일인지 확인**합니다:
  ```
  python scripts/run_cycle.py fetch-tuesday-destination-theme
  ```
  - `"not_due"`(화요일이 아님)면 아래 수요일 확인으로 넘어갑니다.
  - `"no_themes_left"`면 `state/destination_theme_backlog.json`의 테마를 다 다뤘다는 뜻입니다.
    화요일 콘텐츠가 끊기지 않도록, 기존 테마와 같은 결('K-pop테마 투어 2박3일', 'K-뷰티 및 쇼핑투어',
    '한국문화체험 2박3일', '역사·미술관 전시 2박3일', '클럽&펍&바 문화 체험' 같은 "N박N일"/"테마 투어"
    형식)의 새 테마를 에이전트가 직접 구상해 `state.add_destination_theme(theme_id, topic)`으로
    추가한 뒤, 수요일 확인으로 넘어가지 않고 그 테마로 바로 글을 씁니다.
  - `"ok"`면 `{id, topic}` 테마 하나가 나옵니다. TourAPI 장소 데이터가 아니라 일반 지식(및 필요하면
    WebSearch로 실제 존재하는 장소·최신 정보 확인)을 바탕으로, 그 테마에 맞는 여행 코스(보통 2박3일)를
    에이전트가 직접 구성해 글을 씁니다. 코스에 등장하는 구체적인 장소는 이미 발행된 관광지 글과
    자연스럽게 겹칠 수 있으니 문제없고, 사진은 아래 콘텐츠 작성 원칙의 Wikimedia Commons 방식을
    따릅니다.
- 화요일 여행지 테마도 해당 없으면, **오늘이 수요일인지 확인**합니다:
  ```
  python scripts/run_cycle.py fetch-wednesday-food-topic
  ```
  - `"not_due"`(수요일이 아님)면 아래 여행 팁 우선 순서로 넘어갑니다.
  - `"no_topics_left"`면 `state/food_topic_backlog.json`의 주제를 다 다뤘다는 뜻입니다. 수요일
    콘텐츠가 끊기지 않도록, 기존 주제와 같은 결의 새 맛집/음식 주제를 에이전트가 직접 구상해
    `state.add_food_topic(topic_id, topic)`으로 추가한 뒤, 여행 팁 순서로 넘어가지 않고 그 주제로
    바로 글을 씁니다.
  - `"ok"`면 `{id, topic}` 주제 하나가 나옵니다. 일반 지식(및 필요하면 WebSearch)을 바탕으로 글을
    쓰되, 아래 **"맛집/음식 글 추가 원칙"**을 반드시 지킵니다.
- 수요일 맛집/음식 주제도 해당 없으면, **여행 팁 콘텐츠를 먼저 소진**합니다:
  ```
...
- **매주 수요일 맛집/음식 글은 아래 원칙을 추가로 지킨다** (`fetch-wednesday-food-topic`로 나온 주제):
  - **매운 음식·도전적인 메뉴는 미리 경고한다.** 청양고추, 불닭, 마라 등 외국인 기준 맵거나 향이
    강해 호불호가 갈릴 수 있는 메뉴를 소개할 땐, 그 음식 설명 바로 옆에 "이 음식은 맵습니다" 같은
    경고 문구를 눈에 띄게(강조 색 등) 넣고, 순한 대안 메뉴가 있으면 같이 알려준다.
  - **예약 필수 식당은 되도록 추천하지 않는다.** 외국인 여행자는 한국의 예약/웨이팅 문화(전화 예약,
    네이버 예약, 웨이팅 앱 등)에 익숙하지 않을 수 있으므로, 특정 식당을 콕 집어 추천할 땐 예약 없이
    바로 들어갈 수 있는(walk-in) 곳 위주로 고른다. 정말 소개할 가치가 큰 예약 필수 맛집이라면 예외로
    넣되, "예약이 필요하다"는 점과 예약 방법(전화/앱/현장 대기줄 등)을 분명히 적는다.
  - **예약 앱(테이블링, 캐치테이블 등)을 다루는 글은 실제 외국인 이용 가능 여부를 WebSearch로 확인
    후 쓴다.** 한국 번호 인증 필요 여부, 영어 지원 여부, 외국 카드 결제 가능 여부처럼 이 파이프라인이
    미리 알 수 없는 정보이므로, 작성 시점에 최신 정보를 검색해 확인하고, 확인 못 한 부분은 "확인 시점
    기준"이라고 명시하거나 독자가 직접 앱스토어에서 최신 지원 현황을 확인하도록 안내한다.
...
- `destination_theme_backlog.json` — 매주 화요일 여행지 테마 로스터 주제 목록 (`{id, topic}`). 소진되면
  에이전트가 같은 결의 새 테마를 직접 추가한다 (직접 편집도 가능)
- `food_topic_backlog.json` — 매주 수요일 맛집/음식 로스터 주제 목록 (`{id, topic}`). 소진되면 에이전트가
  같은 결의 새 주제를 직접 추가한다 (직접 편집도 가능)
